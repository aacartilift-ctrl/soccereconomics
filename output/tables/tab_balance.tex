...
\footnotesize
\item \textit{Notes:} Pre-treatment (pre-2023--24) covariate means for treated (Big~5)
and control leagues. ``Norm.\ Diff.'' is the normalized difference
\citep{imbens2009recent}: $(\bar{X}_T - \bar{X}_C)/\sqrt{s_T^2 + s_C^2}$.
Values exceeding $|0.25|$ in absolute value (shown in bold) indicate substantive
imbalance warranting further investigation. $N$ is the combined number of
